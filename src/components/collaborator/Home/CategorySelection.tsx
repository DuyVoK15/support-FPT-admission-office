...
const CategorySelection = () => {
  ...
  const postCategoryList = useAppSelector((state)=>state.collab_post.postCategory);
  ...
  return (
    <View style={{ flex: 1, flexDirection: 'row' }}>
      {postCategoryList &&  postCategoryList?.data.map((category, index) => (
        <TouchableOpacity
          key={index}
          ...
          <View style={{ marginTop: 10 }}>
            <Text
              style={{
                ...
                fontSize: 16,
                color: isPriorityChecked[index] ? COLORS.super_dark_orange : 'grey',
              }}
            >
