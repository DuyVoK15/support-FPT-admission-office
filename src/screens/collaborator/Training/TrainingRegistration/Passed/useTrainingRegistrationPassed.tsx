...
import { getAllTrainingCertificateRegistration, getAllTrainingCertificateRegistration_Passed } from '../../../../../features/collaborator/collab.certificateSlice';
...
const useTrainingRegistrationPassed = () => {
  ...
  const fetchTrainingCertificateRegistration = async () => {
    try {
      await dispatch(getAllTrainingCertificateRegistration_Passed({Page: 1,
        PageSize: 1000,
        Sort: 'CreateAt',
        Order: 'DESCENDING',
        Status: TRAINING_CERTI_REGIS_STATUS_ENUM.PASSED,})).then(
        (res) => {}
      );
    } catch (error) {
      console.log(error);
